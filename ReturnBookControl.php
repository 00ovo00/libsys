   ...
   for($i=0; $i<$userNum; $i++) {
       ...
       if($ID == $userRecord[0]){     // 사용자 정보 데이터베이스의 첫번째 열 정보가 ID값이라고 설정했을때
           ...
           if ($userRecord[7] == true)     // 사용자가 대출 가능 상태이면
               ...
               echo("<script>location.href='BorrowDenied_User.html';</script>");
               return false;
           }      
       }
   }
...
function update_user_table($ID)
{
    ...
    for($i=0; $i<$userNum; $i++) {
        ...
        if($ID == $userRecord[0]){     // 사용자 정보 데이터베이스의 첫번째 열 정보가 ID값이라고 설정했을때
            ...
            $query = "update user set LASTRETURNDATE='{$userRecord[10]}' where userid='{$userRecord[0]}'";
            mysql_query($query, $connect);
...
function update_book_table($bNum)
{
 ...
 $rDate; // 예정 반납일자
 $rrDate=date("Y-m-d");       // real return Date 실제 반납 일자
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary"><?echo($ID);?>님 <? echo iconv("EUC-KR", "UTF-8", $resultrow[0]); ?> 을(를) 반납하였습니다</h6>   
                                </div>
                               
                                <div class="card-body">
                                <h6 class="m-0 font-weight-bold text-secondary">반납 정보</h6><br>
                                ...
                                반납 날짜 : <? echo iconv("EUC-KR", "UTF-8", $rrDate); ?><br>
                                연체 일수 : <? echo iconv("EUC-KR", "UTF-8", $overdueCount); ?>
                                </div>  
                        </div>
                    <div class="col-lg-6">
