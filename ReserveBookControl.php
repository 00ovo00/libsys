...
// reserve_record 테이블과 연결
function connect_reserve_record_table($ID, $bNum)
{
    global $connect;

    $query = "insert into RESERVE_RECORD values('', '$ID', '$bNum')";
    ...
    for($i=0; $i<$userNum; $i++) {
        ...
        if($ID == $userRecord[0]){     // 사용자 정보 데이터베이스의 첫번째 열 정보가 ID값이라고 설정했을때
            ...
            if ($userRecord[7] == true)     // 사용자가 대출 가능 상태이면
                ...
                echo("<script>location.href='ReserveDenied_User.html';</script>");
                return false;
            }      
        }
    }
...
function update_book_table($bNum)
{
    ...
    for($i=0; $i<$bookNum; $i++) {
        ...
        if($bNum == $bookRecord[0])       // 도서 정보 데이터베이스의 첫번째 열 정보가 바코드 번호라고 설정했을때
                                        // 도서 정보가 일치하면
            $bookRecord[8] = true;      // 상태를 예약중으로 변경
 ...
 if($isUser == true && $isBook == true) {       // 사용자 정보가 유효한 경우에만
    connect_reserve_record_table($ID, $bNum);       // 예약 승인(예약 데이터를 기록)
    update_book_table($bNum);   // 예약 정보 도서 DB에 업데이트
     // 대출 정보 출력 위한 데이터 로드
    $query="select BNAME, AUTHOR, PUBLISHER from book where BARCODENUM='{$bNum}'";
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary"><?echo($ID);?>님 <? echo iconv("EUC-KR", "UTF-8", $resultrow[0]); ?> 을(를) 예약했습니다</h6>   
                                </div>
                               
                                <div class="card-body">
                                <h6 class="m-0 font-weight-bold text-secondary">예약 정보</h6><br>
                                책제목 : <? echo iconv("EUC-KR", "UTF-8", $resultrow[0]); ?><br>
                                지은이 : <? echo iconv("EUC-KR", "UTF-8", $resultrow[1]); ?><br>
                                출판사 : <? echo iconv("EUC-KR", "UTF-8", $resultrow[2]); ?><br>
                                </div>
                        </div>
                    <div class="col-lg-6">
